...
const BENEFITS = [
  {
    ...
    desc: 'Conecta con empresarios de tu sector y amplía tu red profesional en la región.',
    color: 'bg-blue-500',
  ...
  },
  {
    ...
    desc: 'Accede a eventos exclusivos, ruedas de negocios y espacios de networking.',
  ...
  },
  {
    ...
    desc: 'Defendemos tus intereses ante entidades públicas y privadas de la región.',
    color: 'bg-violet-500',
  ...
  },
  {
    ...
    desc: 'Herramientas, asesoría jurídica y formación continua para crecer.',
    color: 'bg-orange-500',
...
const Input = ({ className = '', ...props }) => (
  <input
    className={`w-full px-4 py-2.5 border-2 border-gray-200 rounded-xl text-sm focus:border-primary-400 focus:outline-none transition-colors hover:border-gray-300 ${className}`}
    {...props}
  />
);
...
const Select = ({ children, className = '', ...props }) => (
  <select
    className={`w-full px-4 py-2.5 border-2 border-gray-200 rounded-xl text-sm focus:border-primary-400 focus:outline-none transition-colors bg-white hover:border-gray-300 ${className}`}
    {...props}
  >
    {children}
  </select>
...
const Textarea = ({ className = '', ...props }) => (
  <textarea
    className={`w-full px-4 py-2.5 border-2 border-gray-200 rounded-xl text-sm focus:border-primary-400 focus:outline-none transition-colors resize-none hover:border-gray-300 ${className}`}
    {...props}
  />
);
              ...
              <div className="flex flex-col items-center gap-2">
                <motion.div
                  ...
                  className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm transition-all duration-300 ${
                    ...
                    step === s.num ? 'bg-primary-500 text-white ring-4 ring-primary-100 shadow-md' :
                    'bg-gray-100 text-gray-400'
                  }`}
                >
                  {step > s.num ? <Check className="w-5 h-5" /> : s.num}
                </motion.div>
                <span className={`text-xs font-semibold hidden sm:block transition-colors duration-300 ${step >= s.num ? 'text-primary-600' : 'text-gray-400'}`}>
                  {s.label}
                </span>
              </div>
              {i < STEPS.length - 1 && (
  ...
  return (
    <div className="pt-20">
      ...
      <AnimatePresence mode="wait">
        ...
        {activeTab === 'pago' && (
          <motion.div key="pago" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
            <PaymentSection />
          </motion.div>
        )}
      </AnimatePresence>
